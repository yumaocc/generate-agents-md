...
`generate-agents-md` 是一个 Codex skill，用来为任意代码仓库生成或更新项目专属的 `AGENTS.md`。
...
这个 skill 会指导 Codex 先读取用户要求，再阅读仓库事实，最后生成真正贴合项目的 `AGENTS.md`。
...
前端开发者通常已经有 Node.js。仓库内置了 Node 安装脚本，发布到 GitHub 后可以直接用：
...
如果你的环境有 Python，也可以使用 Codex skill installer：
...
`generate-agents-md` is a Codex skill for generating or updating project-specific `AGENTS.md` files for any codebase.
...
This skill guides Codex to capture the user's requirements first, inspect the repository second, and then generate an `AGENTS.md` that fits the actual project.
...
Frontend developers usually already have Node.js. This repository includes a Node installer, so after publishing the repo to GitHub you can run:
...
If your environment has Python, you can also use the Codex skill installer:
