...
这个包可以作为普通 npm package 发布，不需要提前在 npm 网站创建项目。首次发布时运行：
...
说明 npm 账号需要开启 2FA，或者使用开启了 bypass 2FA 的 granular access token。最简单的方式是在 npm 账号设置里开启 2FA，然后重新执行：
...
This package can be published like a regular npm package. You do not need to create the package on the npm website first. For the first publish:
...
Your npm account needs 2FA enabled, or you need to publish with a granular access token that can bypass 2FA. The simplest path is to enable 2FA in your npm account settings, then run:
